...
export const uploadMedia = asyncHandler(async (req, res) => {
  if (!req.file) {
    return res.status(400).json(ApiResponse.badRequest("No file uploaded"));
  }
  ...
  res.status(201).json(
    ApiResponse.created(
      {
        ...media,
        path: urlPath, // Return path for frontend compatibility
      },
...
export const deleteMedia = asyncHandler(async (req, res) => {
  ...
  if (!media) {
    return res.status(404).json(ApiResponse.notFound("Media not found"));
  }
  ...
  res.json(ApiResponse.success(null, "Media deleted successfully"));
});
